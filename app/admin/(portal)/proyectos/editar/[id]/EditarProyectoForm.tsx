...
import { updateProyecto, addGalleryImage, removeGalleryImage, type Proyecto } from "../../actions";
import {
  ArrowLeft, Upload, Plus, X, Star, Loader2, Save, Trash2,
  Image as ImageIcon, Eye, MapPin, Calendar, Building2,
...
export default function EditarProyectoForm({ proyecto }: { proyecto: Proyecto }) {
  ...
  const [title, setTitle] = useState(proyecto.title || "");
  const [category, setCategory] = useState(proyecto.category || "");
  const [status, setStatus] = useState<"planning" | "active" | "completed" | "paused">(proyecto.status || "active");
  const [excerpt, setExcerpt] = useState(proyecto.excerpt || "");
  const [description, setDescription] = useState(proyecto.description || "");
  const [clientType, setClientType] = useState(proyecto.client_type || "");
  const [clientName, setClientName] = useState(proyecto.client_name || "");
  const [location, setLocation] = useState(proyecto.location || "");
  const [startDate, setStartDate] = useState(proyecto.start_date || "");
  const [endDate, setEndDate] = useState(proyecto.end_date || "");
  const [tagsInput, setTagsInput] = useState((proyecto.tags || []).join(", "));
  const [metrics, setMetrics] = useState<{ label: string; value: string }[]>(proyecto.metrics || []);
  const [metricLabel, setMetricLabel] = useState("");
  const [metricValue, setMetricValue] = useState("");
  const [featured, setFeatured] = useState(proyecto.featured);
  const [imagePreview, setImagePreview] = useState<string | null>(proyecto.img_url);
  const [coverFile, setCoverFile] = useState<File | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");
  const [gallery, setGallery] = useState<string[]>(proyecto.gallery || []);
  ...
  async function handleGalleryUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fd.set("image", file);
    const result = await addGalleryImage(proyecto.id, fd);
    if (result?.imageUrl) setGallery(prev => [...prev, result.imageUrl!]);
    setGalleryUploading(false);
    e.target.value = "";
  }

  async function handleRemoveGalleryImage(url: string) {
    const result = await removeGalleryImage(proyecto.id, url);
    if (result?.success) setGallery(prev => prev.filter(u => u !== url));
  }

  /* ─── Export ─── */
  const buildExportHtml = () => {
    const safeTitle = (title || "Proyecto").trim();
    return `<!doctype html>
...
<article>${description || ""}</article>
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const result = await updateProyecto(proyecto.id, formData);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Editar Proyecto</h1>
          <p className="text-gray-400 mt-1">{proyecto.title}</p>
        </div>
      </div>
      ...
      <form ref={formRef} action={handleSubmit} className="space-y-8">
        ...
        <section className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start">
            ...
            <div className="text-sm text-gray-500 mt-2 sm:mt-0">
              ...
              <p>Subir nueva imagen reemplazará la actual</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input value={metricValue} onChange={e => setMetricValue(e.target.value)} placeholder="Valor" className={inputCls + " sm:w-40"} />
            <input value={metricLabel} onChange={e => setMetricLabel(e.target.value)} placeholder="Etiqueta" className={inputCls + " flex-1"} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addMetric(); } }} />
        ...
        <section className="bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md space-y-4">
          <h2 className="text-lg font-semibold text-white border-b border-white/10 pb-3 flex items-center gap-2">
            <ImageIcon size={18} className="text-[var(--accent-cyan)]" /> Galería de Imágenes
          </h2>
          <p className="text-sm text-gray-500">Agrega fotos del proyecto: operaciones, resultados, equipo, instalaciones.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {gallery.map((url, i) => (
              <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 group">
                <img src={url} alt={`Gallery ${i + 1}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <button type="button" onClick={() => handleRemoveGalleryImage(url)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-full text-red-300 transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
            <label className="aspect-square rounded-xl border-2 border-dashed border-white/10 hover:border-[var(--accent-cyan)]/40 flex flex-col items-center justify-center cursor-pointer transition-colors group">
              {galleryUploading ? <Loader2 size={24} className="text-[var(--accent-cyan)] animate-spin" /> : <><Plus size={24} className="text-gray-500 group-hover:text-[var(--accent-cyan)] transition-colors" /><span className="text-xs text-gray-500 mt-1">Agregar</span></>}
        ...
        <div className="pt-4 border-t border-white/10">
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            <button type="submit" disabled={submitting} className="w-full sm:w-auto sm:min-w-[180px] inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[var(--accent-cyan)] text-black font-bold hover:bg-[var(--accent-cyan)]/90 transition-colors disabled:opacity-50">
              {submitting ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />} Guardar Cambios
            </button>
          </div>
        </div>
      </form>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 bg-white/5 sticky top-0 z-10">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={handlePreviewPublish} disabled={submitting} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--accent-cyan)] text-black text-sm font-bold hover:bg-[var(--accent-cyan)]/90 disabled:opacity-60">
                  {submitting ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Guardar
                </button>
            ...
            {previewMode === "completo" ? (
              <div className="w-full bg-[var(--bg-dark)]">
                <div className="relative w-full h-[40vh] md:h-[50vh] bg-gray-900 border-b border-white/5 overflow-hidden">
                  {imagePreview ? <img src={imagePreview} alt="Portada" className="w-full h-full object-cover opacity-60" /> : <div className="w-full h-full bg-slate-800" />}
                ...
                <div className="px-6 md:px-10 py-10 md:py-12 max-w-4xl space-y-10">
                  ...
                  {gallery.length > 0 && (
                    <div>
                      <h3 className="flex items-center gap-2 text-sm font-semibold text-gray-300 mb-3"><ImageIcon size={14} className="text-[var(--accent-green)]" /> Galería</h3>
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                        {gallery.map((url, i) => <div key={i} className="aspect-square rounded-xl overflow-hidden bg-gray-900 border border-white/5"><img src={url} alt={`${title} ${i + 1}`} className="w-full h-full object-cover" /></div>)}
                      </div>
                    </div>
                  )}
                </div>
