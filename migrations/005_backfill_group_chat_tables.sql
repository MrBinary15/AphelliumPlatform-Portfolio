-- ============================================================
-- Migration: Backfill group chat schema (idempotent)
-- Purpose: Recover environments where 004 failed before creating tables.
-- ============================================================

-- Helpers (safe even if tables do not exist yet)
...
CREATE INDEX IF NOT EXISTS idx_chat_rooms_task_id ON public.chat_rooms(task_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_room_messages_sender ON public.chat_room_messages(sender_id);
...
CREATE POLICY "chat_rooms_insert" ON public.chat_rooms FOR INSERT TO authenticated
  WITH CHECK (is_admin_or_coordinador(auth.uid()));
...
CREATE POLICY "chat_room_members_select" ON public.chat_room_members FOR SELECT TO authenticated
  USING (
    user_id = auth.uid()
...
CREATE POLICY "chat_room_members_insert" ON public.chat_room_members FOR INSERT TO authenticated
  WITH CHECK (is_admin_or_coordinador(auth.uid()));

CREATE POLICY "chat_room_members_delete" ON public.chat_room_members FOR DELETE TO authenticated
  USING (is_admin_or_coordinador(auth.uid()));

DROP POLICY IF EXISTS "chat_room_messages_select" ON public.chat_room_messages;
DROP POLICY IF EXISTS "chat_room_messages_insert" ON public.chat_room_messages;
DROP POLICY IF EXISTS "chat_room_messages_update" ON public.chat_room_messages;
DROP POLICY IF EXISTS "chat_room_messages_delete" ON public.chat_room_messages;

CREATE POLICY "chat_room_messages_select" ON public.chat_room_messages FOR SELECT TO authenticated
  USING (
    is_chat_room_member(room_id, auth.uid())
    OR is_admin_or_coordinador(auth.uid())
  );

CREATE POLICY "chat_room_messages_insert" ON public.chat_room_messages FOR INSERT TO authenticated
  WITH CHECK (
    sender_id = auth.uid()
    AND (
      is_chat_room_member(room_id, auth.uid())
...
CREATE POLICY "chat_room_messages_update" ON public.chat_room_messages FOR UPDATE TO authenticated
  USING (
    sender_id = auth.uid()
    OR is_admin_or_coordinador(auth.uid())
  );

CREATE POLICY "chat_room_messages_delete" ON public.chat_room_messages FOR DELETE TO authenticated
  USING (
    sender_id = auth.uid()
    OR is_admin_or_coordinador(auth.uid())
  );
